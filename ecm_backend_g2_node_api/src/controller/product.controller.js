...
const update = (req,res) => {
    var {product_id,category_id,name,description,price,quantity,image} = req.body
    var message = {} // Object.keys(message).length == 0 true
    if(category_id == null || category_id == ""){
        message.category_id = "Param category_id required!"
    }
    ...
    var sql = "UPDATE `product` SET category_id = ?, name=?, description=?, price=?, quantity=?, image=? WHERE product_id = ?"
